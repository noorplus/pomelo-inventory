...
-- Invoice numbers are generated only when an invoice is first saved as Draft.
-- No invoice_counters table is used.
...
-- Invoice numbering.
-- The organization row is transaction-locked, so MAX()+1 is serialized per organization.
-- Purchase and Sales sequences are independent because they use separate tables.
...
begin
  ...
  select coalesce(max(invoice_no::integer), 0) + 1
    into v_next
  from public.purchases
  where organization_id = p_organization_id;

  if v_next > 999999 then
    raise exception 'Purchase invoice number limit reached';
  end if;
...
begin
  ...
  if v_next > 999999 then
    raise exception 'Sales invoice number limit reached';
  end if;
...
create index inventory_movements_product_date_idx
  on public.inventory_movements (organization_id, product_id, movement_date desc);
...
create index expenses_category_date_idx
  on public.expenses (expense_category_id, organization_id, expense_date desc);
create index expenses_contact_date_idx
  on public.expenses (contact_id, organization_id, expense_date desc);
...
create index payments_contact_date_idx
  on public.payments (contact_id, organization_id, payment_date desc);
create index payments_type_date_idx
  on public.payments (organization_id, payment_type, payment_date desc);
...
create index payment_allocations_payment_idx
  on public.payment_allocations (payment_id, organization_id);
create index payment_allocations_purchase_idx
  on public.payment_allocations (purchase_id, organization_id);
create index payment_allocations_sale_idx
  on public.payment_allocations (sale_id, organization_id);
create index payment_allocations_expense_idx
  on public.payment_allocations (expense_id, organization_id);

create index account_transactions_org_date_idx
  on public.account_transactions (organization_id, transaction_date desc);
create index account_transactions_contact_date_idx
  on public.account_transactions (contact_id, organization_id, transaction_date desc);
create index account_transactions_reference_idx
  on public.account_transactions (organization_id, reference_type, reference_id);
...
for each row execute function public.set_updated_at();
create trigger stock_set_updated_at before update on public.stock
for each row execute function public.set_updated_at();
...
-- Invoice number + immutability triggers.
...
begin
  ...
  if new.organization_id <> old.organization_id then
    raise exception 'Purchase organization cannot be changed';
  end if;
  if new.invoice_no <> old.invoice_no then
    raise exception 'Purchase invoice number cannot be changed';
  end if;
  if new.created_by <> old.created_by then
    raise exception 'Purchase creator cannot be changed';
  end if;

  if old.status <> new.status then
    if current_user <> 'postgres' then
      raise exception 'Purchase invoice status is controlled by invoice operations';
    end if;
    return new;
  end if;
...
begin
  ...
  if new.organization_id <> old.organization_id then
    raise exception 'Sales organization cannot be changed';
  end if;
  if new.invoice_no <> old.invoice_no then
    raise exception 'Sales invoice number cannot be changed';
  end if;
  if new.created_by <> old.created_by then
    raise exception 'Sales creator cannot be changed';
  end if;

  if old.status <> new.status then
    if current_user <> 'postgres' then
      raise exception 'Sales invoice status is controlled by invoice operations';
    end if;
    return new;
  end if;
...
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = expense_categories.organization_id
    and ou.user_id = (select auth.uid())
));
...
create policy "purchases_update_member" on public.purchases
...
create policy "purchases_delete_member" on public.purchases
...
-- Purchase items: only items belonging to a Draft invoice are writable.
create policy "purchase_items_select_member" on public.purchase_items
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = purchase_items.organization_id
    and ou.user_id = (select auth.uid())
));
create policy "purchase_items_insert_draft" on public.purchase_items
for insert to authenticated with check (exists (
  select 1 from public.organization_users ou
  join public.purchases p
    on p.id = purchase_items.purchase_id
   and p.organization_id = purchase_items.organization_id
  where ou.organization_id = purchase_items.organization_id
    and ou.user_id = (select auth.uid())
    and p.status = 'Draft'
));
create policy "purchase_items_update_draft" on public.purchase_items
for update to authenticated using (exists (
  select 1 from public.organization_users ou
  join public.purchases p
    on p.id = purchase_items.purchase_id
...
create policy "purchase_items_delete_draft" on public.purchase_items
for delete to authenticated using (exists (
  select 1 from public.organization_users ou
  join public.purchases p
    on p.id = purchase_items.purchase_id
...
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = sale_items.organization_id
    and ou.user_id = (select auth.uid())
));
...
-- Stock: readable only; balance is maintained by controlled inventory operations.
create policy "stock_select_member" on public.stock
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = stock.organization_id
    and ou.user_id = (select auth.uid())
));

-- Inventory ledger: readable only; append-only through controlled operations.
create policy "inventory_movements_select_member" on public.inventory_movements
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = inventory_movements.organization_id
    and ou.user_id = (select auth.uid())
));
...
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = payments.organization_id
    and ou.user_id = (select auth.uid())
));
...
-- Payment allocations: read-only to the client. Controlled operations enforce
-- payment type, target type, and remaining balance atomically.
create policy "payment_allocations_select_member" on public.payment_allocations
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = payment_allocations.organization_id
    and ou.user_id = (select auth.uid())
));

-- Account transactions: immutable ledger; read-only to the client.
create policy "account_transactions_select_member" on public.account_transactions
for select to authenticated using (exists (
  select 1 from public.organization_users ou
  where ou.organization_id = account_transactions.organization_id
    and ou.user_id = (select auth.uid())
));
...
comment on table public.stock is 'Current stock balance per organization and product.';
comment on table public.inventory_movements is 'Immutable inventory movement ledger. Quantity is positive; direction determines stock effect.';
comment on table public.payment_allocations is 'Payment allocations. Balance/type integrity is enforced by controlled payment operations.';
comment on table public.account_transactions is 'Immutable accounting ledger. Corrections should use reversing entries.';
