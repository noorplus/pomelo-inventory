...
begin
  ...
  loop
    ...
    if v_alloc.sale_id is not null then
      select total into v_invoice_total from public.sales where id = v_alloc.sale_id for update;
      if not found then raise exception 'Sales invoice not found'; end if;

      if not exists (
        select 1 from public.sales where id = v_alloc.sale_id and organization_id = v_payment.organization_id and status = 'Confirmed'
      ...
      select total into v_invoice_total from public.purchases where id = v_alloc.purchase_id for update;
      if not found then raise exception 'Purchase invoice not found'; end if;

      if not exists (
        select 1 from public.purchases where id = v_alloc.purchase_id and organization_id = v_payment.organization_id and status = 'Confirmed'
      ...
      select amount into v_invoice_total from public.expenses where id = v_alloc.expense_id for update;
      if not found then raise exception 'Expense not found'; end if;

      if not exists (
        select 1 from public.expenses where id = v_alloc.expense_id and organization_id = v_payment.organization_id and status = 'Confirmed'
