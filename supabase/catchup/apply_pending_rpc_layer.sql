...
-- ================= 20260927120000_sale_draft_operations.sql =================
...
-- ================= 20260927130000_returns_aware_outstanding.sql =================
...
-- ================= 20260927140000_allow_duplicate_draft_lines.sql =================
-- Pomelo Inventory - Allow repeated products across draft lines
-- Repository migration only. Do NOT apply to Supabase unless explicitly requested.
--
-- Real invoices repeat the same product on multiple lines (different prices,
-- discounts per lot). The draft RPCs rejected that with "only once" guards.
-- Those guards were a UX simplification, not a load-bearing rule: confirmation
-- aggregates quantities per product, returns walk line rows FIFO, and totals
-- are recomputed from all lines. This migration redefines the four draft
-- RPCs byte-identical except for the removed uniqueness guards. No new
-- tables; per-line validation (quantity, price, discount caps, same-org
-- product, item-id ownership) is unchanged.
--
-- NOTE: databases that never received 20260926210500 / 20260927120000 do not
-- need this file on its own; it is included in the catch-up bundle in order.
